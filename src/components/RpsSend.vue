...
<template>
  <div class="root" style="background-color:white;margin-top:-8px">
    <div v-if="isLoadMore">
      ...
      >
        <div>
          <panel :header="Title" :list="sendData" :type="type" @on-click-item="onItemClick" @on-img-error="onImgError"></panel>        
        </div>
      </scroller>
    </div>

    <div v-if="isLoadMore && sendData.length==0 && showNoData==true">
...
export default {
  ...
  methods: {
    ...
    getContent(flash) {
      ...
      if(!!flash) {
        this.$store.dispatch('rpsSend/clear');
      }

      return this.$store.dispatch('rpsSend/pull').then(ret => {
